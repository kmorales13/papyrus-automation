...
namespace Papyrus.Automation {
	...
	public class BackupManager : Manager {
		...
		public BackupManager(ProcessManager p, RunConfiguration runConfig, Timer keepAliveTimer) {
			_bds = p;
		...
		public void CreateWorldBackup(bool fullCopy, bool archive) {
			string worldPath = Path.Join(RunConfig.BdsPath, "worlds", RunConfig.WorldName);
			string destinationPath = Path.Join(Directory.GetCurrentDirectory(), _tempPath, RunConfig.WorldName);
			...
			if (RunConfig.StopBeforeBackup && RunConfig.EnableKeepAlive && !_bds.IsRunning) {
